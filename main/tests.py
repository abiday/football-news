...
class MainTest(TestCase):
    def test_main_url_is_exist(self):
        ...
        #Apakah SERVER memberikan respons 200 ketika client akses

    def test_main_using_main_template(self):
        response = Client().get('')
        self.assertTemplateUsed(response, 'main.html')
        #Apakah HALAMAN UTAMA mengembalikan kode 200

    def test_nonexistent_page(self):
        response = Client().get('/burhan_always_exists/')
        self.assertEqual(response.status_code, 404)
        #Kalau client akses URL yang gaada ERROR 404
    ...
    def test_news_creation(self):
        ...
        #Ngetest objek yang baru dibuat dengan atribut lalu di cek sama properti yang ada di self itu
